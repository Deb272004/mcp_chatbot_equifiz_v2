...
# """
# graph.py – EQUIFIZ Financial AI Agent (v5.3)

# Key changes from v5.2
# ───────────────────────
# 1. MULTI-ENTITY MCP RESOLUTION — node_mcp_multi_pre_resolve now resolves
#    DB codes for ALL entities (primary + additional_companies) using
#    _targeted_db_lookup(), not the legacy fallback. Every entity gets its
#    own mcp_resolved_codes dict.

# 2. SCHEME/AMC METADATA PRESERVED IN companies[] — node_classify_and_extract
#    carries scheme_name and amc_name from additional_companies into
#    state["companies"] so the resolver has full MF context for every fund,
#    not just the primary one.

# 3. MULTI-ENTITY INJECTION BLOCK — node_mcp_tool_call uses the new
#    _build_injection_block() helper which emits a numbered sub-block per
#    entity when 2+ entities are resolved, giving the MCP server all codes
#    without any internal resolve_mf_scheme round-trip.

# 4. CLASSIFY PROMPT UPDATED — additional_companies now always includes
#    scheme_name and amc_name for MF entities so the LLM faithfully passes
#    through every fund name in comparison/investment queries.

# 5. All v5.2 design decisions (tool-driven param resolution, co_code
#    carry-forward guard, no-param tool pass-through, asyncio fix,
#    early-exit, timeout hardening) are unchanged.

# Pipeline design (v5.3)
# ────────────────────────────────────
# Node A  : classify + extract
#            Detects query_type, entity, intent, relevant_apis (2-3).
#            Also sets is_broad=True if the query is a "full analysis" request.
#            Detects mcp_needed=True for real-time stock/MF queries.
#            For MF comparison/investment: emits scheme_name+amc_name per
#            additional entity so ALL funds get resolved.
...
#            └─ "investment"  → multi_direct_fetch → per_co_summarise
#                               → investment_advisor → END
#                   mcp_needed → mcp_multi_pre_resolve → mcp_tool_call
#                                → mcp_synthesis → END
# """
# from __future__ import annotations
...
# _MCP_NOT_FOUND_SIGNALS = (
#     "not found", "no data", "unavailable", "not available",
#     "no nav data", "no returns data", "no holdings data",
#     "no sector", "no asset", "data not found",
# )
...
#     # Multi-company (comparison / investment / multi-MCP)
#     # Each entry may contain: name, nse_symbol, scheme_name, amc_name,
#     # mcp_resolved_codes, co_code, company_info, raw_api_data, summary, …
#     companies: list[dict]
...
#     scheme comparison, portfolio changes, risk ratios (Alpha/Beta/Sharpe),
#     lumpsum return calculator
#   - Listing schemes / funds offered by an AMC
#   Set FALSE for historical fundamental data (P&L, balance sheet, key ratios, PE).
...
#   market activity    → "get_mf_market_activity"
#   lumpsum calculator → "get_lumpsum_returns"
#   AMC categories     → "get_fund_categories"
#   (leave empty if unclear)

# is_broad — set true ONLY for: "full analysis", "complete report", "deep dive",
#   "everything about X", "all financials", "comprehensive report".
#   Default is FALSE. mcp_needed queries are never broad by default.

# Entity extraction rules:
# - Resolve brand names: "Maggi" → "Nestle India", "Jio" → "Reliance Industries"
# - Use conversation history to resolve pronouns ("it", "the company", "them")
# - For MF queries extract BOTH scheme_name (full scheme name with plan suffix
#   like "(G)", "(D)", "(G)-Direct") AND amc_name (fund house).
# - CRITICAL for comparison/investment queries — emit ALL mentioned entities:
#     primary       = first entity mentioned
#     additional_companies = every subsequent entity, one object each
#   For MF entities ALWAYS populate scheme_name AND amc_name — do NOT leave
#   them null. The resolver depends on these fields to look up DB codes.
#   Example — "compare ABSL Large & Mid Cap vs HDFC Income Fund (G)":
#     primary.scheme_name                    = "Aditya Birla SL Large & Mid Cap Fund (G)"
#     primary.amc_name                       = "Aditya Birla Sun Life Mutual Fund"
#     additional_companies[0].scheme_name    = "HDFC Income Fund (G)"
#     additional_companies[0].amc_name       = "HDFC Mutual Fund"
#   Example — "TCS vs Infosys vs Wipro":
#     primary.company_name                   = "Tata Consultancy Services"
#     primary.nse_symbol                     = "TCS"
#     additional_companies[0].company_name   = "Infosys"
#     additional_companies[0].nse_symbol     = "INFY"
#     additional_companies[1].company_name   = "Wipro"
#     additional_companies[1].nse_symbol     = "WIPRO"
# - If query is about an AMC's schemes set amc_name and leave company_name null.

# API selection (relevant_apis — pick 2-3, ignored when is_broad or mcp_needed):
#   pe_ratio / valuation  → ["daily_ratios", "key_ratios"]
#   eps / earnings        → ["daily_ratios", "key_ratios", "quarterly_results"]
#   revenue / profit      → ["quarterly_results", "profit_and_loss"]
#   balance sheet / debt  → ["balance_sheet", "key_ratios"]
...
#     _AMC_NAMES = {
#         "sbi", "hdfc", "icici", "axis", "nippon", "mirae", "kotak",
#         "uti", "ppfas", "parag parikh", "quant", "tata", "dsp",
#         "franklin", "aditya birla", "absl", "edelweiss", "bandhan",
#         "motilal", "canara", "navi", "pgim", "whiteoak", "360 one",
#     }
#     _LISTING_WORDS = {
#         "scheme", "schemes", "fund", "funds", "provide", "provides",
#         "offer", "offers", "available", "list", "show", "what",
#     }
#     if not keyword_mcp:
#         words   = set(query_lower.split())
#         has_amc = any(amc in query_lower for amc in _AMC_NAMES)
#         if has_amc and bool(words & _LISTING_WORDS):
#             keyword_mcp = True
...
#         # ── Build companies list — preserve scheme_name + amc_name ────────
#         additional = result.get("additional_companies") or []
#         if additional:
#             companies = []
#             for c in additional:
...
#                 amc    = c.get("amc_name")
#                 name   = scheme or c.get("company_name") or ""
#                 companies.append({
#                     "name":        name,
...
# # ═══════════════════════════════════════════════════════════════════════════════
# # DB Helpers shared by MCP pre-resolution nodes
# # ═══════════════════════════════════════════════════════════════════════════════
...
# ) -> Optional[dict]:
#     """
#     Generic fuzzy lookup against any table/column in PARAM_TO_TABLE_MAP.

#     For scheme_master the full row is returned (including mf_cocode) so
#     both codes are resolved in a single query.  Returns the best-matching
#     row dict, or None if the best score is below 60.
#     """
#     try:
#         conn = psycopg2.connect(**DB_CONFIG)
#         cur  = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
#         console.print(f"🔍 [DB] table='{table}' entity='{entity}'")
...
#         if best_match and best_score >= 60:
#             console.print(
#                 f"  ✅ match='{best_match[name_col]}' id={best_match[id_col]} "
#                 f"score={best_score}"
#             )
#             return best_match
...
# ) -> dict:
#     """
#     Legacy MF code resolver — falls back through scheme_name → amc_name →
#     entity_name.  Used only as a last resort by node_mcp_pre_resolve.
#     """
#     resolved: dict = {}
#     try:
#         conn = psycopg2.connect(**DB_CONFIG)
...
#             )
#             best_score, best_scheme = 0, None
#             for s in [dict(r) for r in cur.fetchall()]:
#                 nl = (s.get("sch_name") or "").lower()
#                 ql = search_scheme.lower()
#                 score = max(
#                     fuzz.token_set_ratio(ql, nl),
#                     fuzz.partial_ratio(ql, nl),
#                 )
#                 if score > best_score:
#                     best_score, best_scheme = score, s
#             if best_scheme and best_score >= 70:
...
#                 resolved["scheme_category"] = best_scheme.get("category")
#                 console.print(
#                     f"  [legacy] scheme='{best_scheme['sch_name']}' "
#                     f"schcode={resolved['mf_schcode']} score={best_score}"
#                 )
...
#             cur.execute("SELECT mf_cocode, lname, nameamc FROM fund_house")
#             best_score, best_house = 0, None
#             for h in [dict(r) for r in cur.fetchall()]:
#                 nl = (h.get("lname") or "").lower()
#                 ns = (h.get("nameamc") or "").lower()
#                 ql = search_amc.lower()
#                 score = max(
...
#         conn.close()
#     except Exception as e:
#         console.print(f"  [_resolve_mf_codes_from_db] DB error: {e}")
...
# ) -> dict:
#     """
#     Resolve all relevant DB codes for a single entity.

#     Resolution order:
#       1. scheme_master  → mf_schcode + mf_cocode  (for MF scheme entities)
#       2. fund_house     → mf_cocode               (for AMC-only entities)
#       3. company_master → co_code                 (only if tool requires it)

#     Returns a dict with: mcp_resolved_codes, resolved_scheme_name (optional),
#     co_code (optional), company_info (optional), nse_symbol (optional).
#     """
#     codes:  dict = {}
#     result: dict = {"name": name}

#     # ── 1. Try scheme_master ──────────────────────────────────────────────
#     search_scheme = scheme_name or name
#     if search_scheme:
#         cfg   = PARAM_TO_TABLE_MAP["mf_schcode"]
#         match = _targeted_db_lookup(
#             entity   = search_scheme,
#             table    = cfg["table"],
#             name_col = cfg["column"],
#             id_col   = cfg["id_field"],
#         )
...
#             result["resolved_scheme_name"] = match.get("sch_name", search_scheme)
#             console.print(
#                 f"  💎 [{name}] mf_schcode={codes['mf_schcode']} "
#                 f"mf_cocode={codes.get('mf_cocode')} (scheme_master)"
#             )

#     # ── 2. If still no mf_cocode, try fund_house ──────────────────────────
#     if not codes.get("mf_cocode"):
#         search_amc = amc_name or name
#         if search_amc:
...
#                 codes["mf_cocode"] = int(match["mf_cocode"])
#                 console.print(
#                     f"  💎 [{name}] mf_cocode={codes['mf_cocode']} (fund_house)"
#                 )

#     # ── 3. Stock co_code — only when the tool requires it ─────────────────
#     if needs_co_code and not codes.get("co_code"):
...
#             })
#             console.print(
#                 f"  💎 [{name}] co_code={codes['co_code']} (company_master)"
#             )
...
# # ═══════════════════════════════════════════════════════════════════════════════
# # MCP Pre-Resolution Node  — single entity (v5.3)
# # ═══════════════════════════════════════════════════════════════════════════════

# def node_mcp_pre_resolve(state: AgentState) -> AgentState:
#     """
#     Tool-Driven Parameter Resolution for single-entity MCP queries (v5.3).

#     Strategy
#     ────────
#     1. Use mcp_tool_hint to look up required params via TOOL_REQUIRED_PARAMS.
#     2. No-param tools → skip all DB resolution.
#     3. Unknown tool → fall back to equifiz_tools Chroma collection.
#     4. For each required param call _targeted_db_lookup().
#     5. co_code carry-forward guard: only injected when the tool needs it.
#     6. Last resort: legacy _resolve_mf_codes_from_db().
#     """
#     console.print("[Node MPR] Single-entity MCP pre-resolve (v5.3)")
...
#     # ── Step 1: Determine required params ────────────────────────────────
#     required_params: Optional[list[str]] = None

#     if tool_hint:
#         required_params = _get_required_params_for_tool(tool_hint)
#         if required_params is not None:
#             console.print(f"📋 Tool='{tool_hint}' requires: {required_params}")
#         else:
#             console.print(f"  Tool '{tool_hint}' not in registry — trying Chroma")
#     else:
#         console.print("  No mcp_tool_hint — trying Chroma")

#     # ── Step 2: No-param tool ─────────────────────────────────────────────
#     if required_params is not None and len(required_params) == 0:
#         console.print(f"⚡ No-param tool '{tool_hint}' — skipping DB resolution")
#         state["mcp_resolved_codes"] = {}
#         return state

#     # ── Step 3: Chroma fallback for unknown tools ─────────────────────────
#     if required_params is None:
#         try:
#             import chromadb
#             client     = chromadb.PersistentClient(path="./chroma_db")
...
#                 chroma_params = [p.strip() for p in raw_params.split(",") if p.strip()]
#                 console.print(
#                     f"🎯 Chroma hit: tool='{chroma_tool}' params={chroma_params}"
#                 )
#                 mappable = [p for p in chroma_params if p in PARAM_TO_TABLE_MAP]
#                 if mappable:
#                     required_params = mappable
...
#                         required_params = inferred
#             else:
#                 console.print("  No Chroma match")
#         except Exception as e:
#             console.print(f"  Chroma query failed: {e}")

#     # ── Step 4: Resolve each required param ──────────────────────────────
#     if required_params:
#         for param in required_params:
#             if param not in PARAM_TO_TABLE_MAP:
#                 continue
...
#                     resolved_codes.setdefault("mf_cocode", int(match["mf_cocode"]))
#             else:
#                 console.print(f"  Could not resolve '{param}' for '{entity}'")

#     # ── Step 5: Legacy fallback ───────────────────────────────────────────
#     if not resolved_codes:
#         console.print("🔄 Fallback: _resolve_mf_codes_from_db()")
#         mf = _resolve_mf_codes_from_db(
#             scheme_name = state.get("mcp_scheme_name"),
#             amc_name    = state.get("mcp_amc_name"),
#             entity_name = entity,
#         )
#         resolved_codes.update(mf)
#         if mf.get("scheme_name"):
#             state["mcp_scheme_name"] = mf["scheme_name"]
#         if mf.get("amc_name"):
#             state["mcp_amc_name"] = mf["amc_name"]
...
#     if state.get("co_code") and tool_needs_co_code:
#         resolved_codes.setdefault("co_code", state["co_code"])
#         console.print(f"  co_code={state['co_code']} carried (tool requires it)")
#     elif state.get("co_code") and not tool_needs_co_code:
#         console.print(
#             f"  ⚠️  co_code={state['co_code']} IGNORED — tool does not need it"
#         )

#     state["mcp_resolved_codes"] = resolved_codes
#     console.print(f"🏁 Resolved codes: {resolved_codes}")
#     return state


# # ═══════════════════════════════════════════════════════════════════════════════
# # MCP Pre-Resolution Node  — multi-entity (v5.3)
# # ═══════════════════════════════════════════════════════════════════════════════

# def node_mcp_multi_pre_resolve(state: AgentState) -> AgentState:
#     """
#     v5.3 — Resolve DB codes for ALL entities in a multi-entity MCP query.

#     Builds an entity list from:
#       • state["mcp_scheme_name"] / state["mcp_amc_name"] (primary)
#       • state["companies"]  (all additional entities from Node A,
#                              each carrying scheme_name + amc_name)

#     Each entity is resolved in parallel via _resolve_entity_codes().
#     Every resolved company dict carries its own ``mcp_resolved_codes``
#     so _build_injection_block() can emit a numbered block per fund.
#     """
#     console.print("[Node MMPR v5.3] MCP multi-entity pre-resolve")
...
#     for c in (state.get("companies") or []):
#         # Prefer scheme_name as the canonical entity name for MF entities
...
#     console.print(
#         f"  Entities to resolve ({len(entities)}): "
...
#             needs_co_code = needs_co_code,
#         )

#     name_to_result: dict[str, dict] = {}
#     with ThreadPoolExecutor(max_workers=min(6, len(entities))) as executor:
#         futures = {
#             executor.submit(_resolve_one, e): e["name"] for e in entities
#         }
#         for future in as_completed(futures):
#             name = futures[future]
#             try:
...
# # ═══════════════════════════════════════════════════════════════════════════════
# # Injection block builder (shared by both MCP tool call paths)
# # ═══════════════════════════════════════════════════════════════════════════════
...
#     Single entity  → flat block  (mf_schcode=..., mf_cocode=..., co_code=...)
#     Multi-entity   → numbered sub-blocks, one per entity, so the MCP server
#                      knows exactly which code belongs to which fund/company.

#     Returns "" when no codes are available (no-param / query-based tools).
#     """
#     companies = [c for c in (state.get("companies") or []) if c.get("mcp_resolved_codes")]
...
#             if codes.get("mf_schcode"):
#                 lines.append(f"  mf_schcode={codes['mf_schcode']}")
#             if codes.get("mf_cocode"):
#                 lines.append(f"  mf_cocode={codes['mf_cocode']}")
#             if codes.get("co_code"):
#                 lines.append(f"  co_code={codes['co_code']}")
#         if lines:
#             return "<PRE_RESOLVED>\n" + "\n".join(lines) + "\n</PRE_RESOLVED>"
#         return ""

#     # ── Single-entity path (original behaviour) ───────────────────────────
#     resolved_codes = state.get("mcp_resolved_codes") or {}
#     scheme_name    = state.get("mcp_scheme_name", "")
#     amc_name       = state.get("mcp_amc_name", "")
...
# def node_mcp_tool_call(state: AgentState) -> AgentState:
#     """
#     Execute the MCP client query with pre-resolved codes injected (v5.3).

#     Uses _build_injection_block() which handles both single- and multi-entity
#     payloads transparently.  Empty injection block means the tool is
#     query-based — the user query is sent as-is.
#     """
#     console.print("[Node MTC v5.3] MCP tool call")
...
#         state["error"]               = None
#         console.print(
#             f"📥 [MCP RECEIVE] {len(state['mcp_raw_result'])} chars"
#         )
...
# def node_direct_api_fetch(state: AgentState) -> AgentState:
#     console.print("[Node D] Direct API fetch")
#     co_code = state.get("co_code")
#     if not co_code:
#         state["raw_api_data"] = {}
#         state["error"]        = f"co_code not resolved for '{state.get('extracted_entity')}'"
...
#     with ThreadPoolExecutor(max_workers=min(4, len(apis))) as executor:
#         futures = {
#             executor.submit(_fetch_one_raw, co_code, ak, ft): (ak, ft)
#             for ak, ft in apis
#         }
#         for future in as_completed(futures):
#             try:
...
#                     raw_data[label] = records
#             except Exception as e:
#                 ak, ft = futures[future]
#                 console.print(f"  Executor error [{ak}]: {e}")
...
#             except Exception as e:
#                 ak, ft = futures[future]
#                 console.print(f"  [{name}] executor error [{ak}]: {e}")
...
#     state["companies"] = [
#         results_map.get(name, {"name": name}) for name, _ in to_resolve
#     ]
#     return state
...
#             executor.submit(
#                 _fetch_and_ingest_one,
#                 co_code, co_code_str, ak, ft, force_refresh
#             ): (ak, ft)
#             for ak, ft in _ALL_FINANCIAL_APIS
#         }
#         for future in as_completed(futures):
#             try:
#                 label, status = future.result()
...
#             except Exception as e:
#                 ak, ft = futures[future]
#                 failed.append(f"{ak}_{ft}" if ft else ak)
#                 console.print(f"  Executor error: {e}")
#     return ingested, failed
...
#     state["raw_api_data"] = {k: "\n".join(v) for k, v in chunks_by_api.items()}
#     console.print(
#         f"  → {len(results)} chunks across {len(chunks_by_api)} APIs"
#     )
#     return state
...
#     company_info = state.get("company_info") or {}
#     company_name = (
#         company_info.get("companyname") or state.get("extracted_entity", "the company")
#     )
#     intent = state.get("intent", "general")

#     def _summarise_one(label: str, records: Any) -> tuple[str, str]:
#         raw_text = records[:3000] if isinstance(records, str) else _records_to_text(records, label)[:3000]
#         prompt   = PER_API_SUMMARY_PROMPT.format(
#             api_label=label, company_name=company_name,
...
#             except Exception as e:
#                 updated[idx] = companies[idx]
#                 console.print(f"  Summarise executor error idx={idx}: {e}")
...
#    points, hyphens as list markers, asterisks, or any special formatting
#    characters. Do NOT use the rupee symbol — write "Rs" instead.
# 3. Provide sector comparison where relevant.
# 4. Highlight if valuation looks expensive or cheap vs sector.
# 5. Use Indian number formatting (Rs Cr — spell out "Rs").
# 6. Be honest if data is unavailable.
# 7. Keep under 250 words unless detail is explicitly requested.
...
# ## Instructions
# 1. Answer the conceptual/general question clearly and concisely.
# 2. Write in plain prose paragraphs only. Do NOT use any markdown.
#    Do NOT use the rupee symbol — write "Rs" instead.
# 3. Use simple language; give Indian market examples where useful.
# 4. If context docs don't cover the topic, answer from general knowledge.
# 5. Keep under 200 words.
# """
...
# ## Instructions
# 1. Write in flowing prose paragraphs only. Do NOT use any markdown formatting.
#    Do NOT use the rupee symbol — write "Rs" instead.
# 2. One opening paragraph naming all entities and the comparison context.
# 3. One paragraph each covering valuation, profitability, growth, and safety/debt.
# 4. A closing verdict paragraph naming the stronger pick with reasoning.
# 5. Weave all numbers into sentences. Use Indian number formatting (Rs Cr).
# 6. Be honest if specific data is missing.
# 7. Keep under 400 words unless detail is explicitly requested.
...
# 1. Write in flowing prose paragraphs only. Do NOT use any markdown formatting.
#    Do NOT use the rupee symbol — write "Rs" instead.
# 2. One paragraph per entity covering PE, PB, ROE, EPS, revenue trend,
#    debt/equity, and MCAP — all woven naturally into sentences.
# 3. One paragraph ranking all entities from most to least attractive,
#    with clear qualitative reasoning.
# 4. State clearly in prose what data was unavailable for any entity.
# 5. Use Indian number formatting (Rs Cr — spell out "Rs").
# 6. Keep under 500 words.
...
#     partial_note = (
#         "Data note: live data unavailable for "
#         + ", ".join(partial_cos)
#         + ". Analysis may be incomplete."
#     ) if partial_cos else ""
...
Node A  : classify + extract
           ...
           │
           ├─ "company"
           │    ├─ mcp_needed=True  → mcp_pre_resolve → mcp_tool_call
           │    │                     → mcp_synthesis → END
           │    ├─ is_broad=False   → symbol_resolution → direct_api_fetch
           │    │                     → summarise_apis → synthesis → END
           │    └─ is_broad=True    → symbol_resolution → full_ingest
           │                          → vector_retrieval → summarise_apis
           │                          → synthesis → END
           │
           ├─ "comparison"
           ...
           └─ "investment"
                ├─ mcp_needed=True  → mcp_multi_pre_resolve → mcp_tool_call
                │                     → mcp_synthesis → END
                └─ (default)        → multi_direct_fetch → per_co_summarise
                                      → investment_advisor → END
"""
from __future__ import annotations
...
# ── Intent → API mapping (fundamental data path only) ────────────────────────
...
# schema, not any tool's business logic. Add rows only when you add a new
# DB table / code type.
...
# Lower = stricter (fewer false positives). Tune against your embeddings.
# Distance is 1 - cosine_similarity, so 0.45 ≈ similarity >= 0.55.
TOOL_SCORE_THRESHOLD = 0.45
    ...
    try:
        ...
        for tool_id, meta, dist in zip(
            ...
            tools.append({
                ...
                "required_parameters": req_params,   # list[str]
...
{{
  ...
  "primary": {{
    ...
    "scheme_name":  "<MF scheme full name or null>",
    "amc_name":     "<AMC / fund house full name or null>",
    "intent":       "<pe_ratio|eps|mcap|quarterly_results|profit_and_loss|balance_sheet|key_ratios|daily_ratios|general>",
    "report_type":  "<s or c>",
    "relevant_apis": ["<api_key_1>", "<api_key_2>"]
  }},
  "additional_companies": [
...
mcp_needed:
  ...
  Set FALSE for historical fundamental data (P&L, balance sheet, key ratios, PE)
  that can be served from the internal equifiz API.
...
is_broad:
  ...
  mcp_needed queries are never broad.
...
API selection (relevant_apis — for non-MCP paths, pick 2-3):
  pe_ratio/valuation  → ["daily_ratios", "key_ratios"]
  eps/earnings        → ["daily_ratios", "key_ratios", "quarterly_results"]
  revenue/profit      → ["quarterly_results", "profit_and_loss"]
  balance sheet/debt  → ["balance_sheet", "key_ratios"]
...
def node_classify_and_extract(state: AgentState) -> AgentState:
    ...
    # Chroma score gate: if any tool matched, mcp_needed is True
    ...
    try:
        ...
        state["query_type"]    = result.get("query_type", "general")
        state["is_broad"]      = result.get("is_broad", False) or keyword_broad
        # Chroma gate overrides LLM if it caught something
        ...
        if raw_apis and not state["is_broad"] and not state["mcp_needed"]:
            ...
            state["relevant_apis"] = INTENT_API_MAP.get(
                state["intent"], _DEFAULT_APIS
            )
...
def node_symbol_resolution(state: AgentState) -> AgentState:
    console.print("[Node SR] Symbol resolution")
    resolved = _resolve_single(
        state.get("extracted_entity", ""),
        state.get("nse_symbol"),
    )
    ...
    state["nse_symbol"]   = resolved.get("nse_symbol")
    console.print(
        f"  → co_code={state.get('co_code')} "
        f"for '{state.get('extracted_entity')}'"
    )
    return state
...
def _get_required_params_for_tool(tool_hint: str, matched_tools: list[dict]) -> list[str]:
    ...
    (i.e. params we can resolve from the DB). Params like 'query',
    'limit', 'top' etc. are ignored — the MCP server handles those.
    """
    if not tool_hint:
        return []

    # Check in-memory matched tools first (fastest)
    ...
    # Fallback: direct Chroma lookup by tool name
...
def _resolve_entity_codes(
    ...
    For each param in required_params (e.g. ['mf_schcode'] or ['co_code']):
      - Look up PARAM_TO_TABLE_MAP for the DB table/column
      - Run _targeted_db_lookup with the best available entity string

    Returns a dict with:
      mcp_resolved_codes: {param: value, ...}
      resolved_scheme_name: str (optional)
      co_code, company_info, nse_symbol (for stock entities)
    """
    codes:  dict = {}
    result: dict = {"name": name}
    ...
    # compare_schemes uses 'mf_schcodes' (plural) — treat same as mf_schcode
...
# ═══════════════════════════════════════════════════════════════════════════════
# MCP Pre-Resolution — single entity (v6.0)
# ═══════════════════════════════════════════════════════════════════════════════
...
def node_mcp_pre_resolve(state: AgentState) -> AgentState:
    ...
    1. Get required_params from Chroma (via _get_required_params_for_tool)
    2. If no DB params needed → no-param tool, skip resolution
    3. Otherwise call _resolve_entity_codes for each DB param
    """
    console.print("[Node MPR] Single-entity MCP pre-resolve (v6.0 dynamic)")
...
# ═══════════════════════════════════════════════════════════════════════════════
# MCP Pre-Resolution — multi-entity (v6.0)
# ═══════════════════════════════════════════════════════════════════════════════
...
def node_mcp_multi_pre_resolve(state: AgentState) -> AgentState:
    ...
    Special case: compare_schemes requires mf_schcodes (plural, comma-separated).
    After resolving all entities, we merge their individual mf_schcode values
    into a single comma-separated mf_schcodes string in the primary resolved_codes.
    """
    console.print("[Node MMPR] Multi-entity MCP pre-resolve (v6.0 dynamic)")
...
def _build_injection_block(state: AgentState) -> str:
    ...
    Single entity  → flat block
    Multi-entity   → numbered sub-blocks (one per entity)
    compare_schemes → single block with comma-separated mf_schcodes

    Returns "" for no-param tools.
    """
    companies = [
...
def node_mcp_tool_call(state: AgentState) -> AgentState:
    console.print("[Node MTC] MCP tool call (v6.0)")
...
# ═══════════════════════════════════════════════════════════════════════════════
# Shared API helpers (fundamental data path)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ── Direct API fetch (single company, no ChromaDB) ───────────────────────────
    ...
    with ThreadPoolExecutor(max_workers=min(4, len(apis))) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                ak, ft = futures[future]
                console.print(f"  [{name}] error [{ak}]: {e}")
    return {
        "name":            name,
        "co_code":         co_code,
        "company_info":    resolved["company_info"],
...
def node_multi_direct_fetch(state: AgentState) -> AgentState:
    ...
    with ThreadPoolExecutor(max_workers=min(4, max(len(to_resolve), 1))) as executor:
        futures     = {
            executor.submit(_resolve_and_direct_fetch, name, sym, apis): name
            for name, sym in to_resolve
        }
...
# ── Full ChromaDB ingestion (is_broad=True only) ──────────────────────────────
...
def _chunks_are_fresh(co_code_str: str, api_name: str, fin_type: str) -> bool:
    try:
        ...
        if fin_type:
            clauses.append({"fin_type": {"$eq": fin_type}})
        existing = col.get(where={"$and": clauses}, include=["metadatas"])
        return len(existing.get("ids", [])) > 0
    except Exception:
        return False
...
def _ingest_chunks(text: str, api_name: str, co_code: str, fin_type: str = "") -> int:
    if not text.strip():
        return 0
    col     = vs._get_collection()
    clauses = [{"api_name": {"$eq": api_name}}, {"co_code": {"$eq": co_code}}]
    if fin_type:
        clauses.append({"fin_type": {"$eq": fin_type}})
    try:
        stale = col.get(where={"$and": clauses}, include=["metadatas"])
        if stale["ids"]:
            col.delete(ids=stale["ids"])
    except Exception as e:
        console.print(f"  Stale delete failed [{api_name}]: {e}")
...
def _fetch_and_ingest_one(
    ...
    n = _ingest_chunks(
        _records_to_text(raw, label),
        api_name=label, co_code=co_code_str, fin_type=fin_type or "",
    )
    return label, "ok"
...
def node_vector_retrieval(state: AgentState) -> AgentState:
    ...
    for r in results:
        ...
        chunks_by_api.setdefault(key, []).append(r["document"])
    state["raw_api_data"] = {k: "\n".join(v) for k, v in chunks_by_api.items()}
    console.print(f"  → {len(results)} chunks across {len(chunks_by_api)} APIs")
    return state
...
Extract only figures directly relevant to the user's intent.
Return 3-6 bullet points, each under 20 words.
Use Indian number formatting (Rs Cr). Factual only — no commentary.
Output ONLY the bullet points, nothing else.
"""
...
def node_summarise_api_responses(state: AgentState) -> AgentState:
    ...
    def _summarise_one(label: str, records: Any) -> tuple[str, str]:
        raw_text = (
            records[:3000] if isinstance(records, str)
            else _records_to_text(records, label)[:3000]
        )
        prompt = PER_API_SUMMARY_PROMPT.format(
            api_label=label, company_name=company_name,
            intent=intent, raw_text=raw_text,
    ...
    summaries: dict[str, str] = {}
    with ThreadPoolExecutor(max_workers=min(4, max(len(raw_data), 1))) as executor:
        futures = {
            executor.submit(_summarise_one, lbl, recs): lbl
            for lbl, recs in raw_data.items()
        }
...
Extract: PE, EPS, Revenue, Profit, ROE, Debt/Equity, MCAP, Dividend Yield.
Return 5-8 bullet points, each under 20 words.
Use Indian number formatting (Rs Cr). Factual only.
Output ONLY the bullet points, nothing else.
"""
...
def node_per_company_summarise(state: AgentState) -> AgentState:
    ...
    updated = [None] * len(companies)
    with ThreadPoolExecutor(max_workers=min(4, max(len(companies), 1))) as executor:
        futures = {executor.submit(_summarise, c): i for i, c in enumerate(companies)}
        for future in as_completed(futures):
            idx = futures[future]
            try:
                ...
                updated[idx] = companies[idx]
                console.print(f"  Executor error idx={idx}: {e}")
...
def _networks_busy_reply(user_query: str, entity: str) -> str:
    BUSY = (
        "You are EQUIFIZ. The user asked: \"{q}\". "
        "All live APIs failed for \"{e}\". "
        "Respond politely in under 60 words: apologise, networks are busy, suggest retry."
    )
    try:
        return _llm_text(BUSY.format(q=user_query, e=entity))
    except Exception:
        return f"Sorry, data networks are busy for {entity}. Please try again shortly."
...
## Instructions
1. Answer concisely using the summarised data above.
2. Plain prose only — no markdown, no bullets, no tables, no special characters.
   Do NOT use the rupee symbol — write "Rs" instead.
3. Sector comparison where relevant. Highlight expensive/cheap valuation.
4. Indian number formatting (Rs Cr). Honest about missing data.
5. Under 250 words unless detail is requested.
"""
...
def node_synthesis(state: AgentState) -> AgentState:
    ...
    if len(raw_data) < len(relevant_apis):
        missing      = len(relevant_apis) - len(raw_data)
        partial_note = (
            f"Data note: {missing} of {len(relevant_apis)} sources "
...
## Instructions
1. Flowing prose only — no markdown, no bullets, no tables.
   Do NOT use the rupee symbol — write "Rs" instead.
2. Opening paragraph: name all entities + comparison context.
3. One paragraph each: valuation, profitability, growth, safety/debt.
4. Closing verdict: strongest pick with reasoning.
5. Indian number formatting (Rs Cr). Honest about missing data.
6. Under 400 words. End with: This is not financial advice.
"""
...
def node_comparison_synthesis(state: AgentState) -> AgentState:
    ...
    partial_note = (
        "Data note: sources temporarily unavailable for "
        + ", ".join(partial_cos) + "."
    ) if partial_cos else ""
...
## Instructions
1. Flowing prose only — no markdown. Do NOT use the rupee symbol — write "Rs".
2. One paragraph per entity: PE, PB, ROE, EPS, revenue trend, debt/equity, MCAP.
3. One paragraph ranking all entities most-to-least attractive with reasoning.
4. State clearly what data was unavailable for any entity.
5. Indian number formatting (Rs Cr). Under 500 words.
...
def node_investment_advisor(state: AgentState) -> AgentState:
    ...
    partial_note = (
        "Data note: live data unavailable for "
        + ", ".join(partial_cos) + ". Analysis may be incomplete."
    ) if partial_cos else ""
...
#         return "general"
#     if qt == "company":
#         return "mcp_direct" if mcp else "company"
#     if qt in ("comparison", "investment"):
#         if mcp:
#             return "multi_mcp"
#         if is_broad and qt == "comparison":
...
def route_after_classify(state: AgentState) -> str:
    ...
    # NEW: If a tool was matched, go to MCP path regardless of query type
    if mcp:
        if qt in ("comparison", "investment"):
            return "multi_mcp"
        return "mcp_direct"
    ...
    if qt in ("comparison", "investment"):
        return "multi_broad" if state.get("is_broad") else "multi_fast"
        
    return "company"


def route_after_symbol(state: AgentState) -> str:
    return "full_ingest" if state.get("is_broad") else "direct_api_fetch"
...
def build_graph() -> Any:
    ...
    g.add_conditional_edges(
        "symbol_resolution",
        route_after_symbol,
        {"direct_api_fetch": "direct_api_fetch", "full_ingest": "full_ingest"},
    )
    ...
    g.add_conditional_edges(
        "per_company_summarise",
        route_after_multi,
        {"comparison": "comparison_synthesis", "investment": "investment_advisor"},
    )
    g.add_edge("comparison_synthesis", END)
    g.add_edge("investment_advisor",   END)

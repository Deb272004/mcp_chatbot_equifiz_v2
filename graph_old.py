...
# """
# graph.py — EQUIFIZ Financial AI Agent (v10.1)

# Key changes from v10.0
# ─────────────────────────────────────────────────────────────────────
# 1. ROUTER FIX — route_after_classify now upgrades mcp_needed=True
#    whenever TVS found matched_tools for ANY intent.  Prevents the
#    "any tools matched → still goes to general handler" regression.

# 2. TVS FIX — _search_for_intent returns a brand-new IntentItem dict
#    instead of mutating the input in place.  Eliminates the LangGraph
#    state-snapshot / thread-mutation race that caused matched_tools to
#    appear empty in the router even after TVS found tools.

# 3. DECOMPOSE INTENTS PROMPT hardened — explicit ENTITY TYPE RULES
#    with negative examples stop the LLM from hallucinating specific
#    ETF/scheme names when the user said "SBI Mutual Fund".

# 4. TVS fallback tightened — error path in the ThreadPoolExecutor
#    also returns a fresh IntentItem so matched_tools is always a list,
#    never missing from the dict.

# 5. Router debug prints added (remove after confirming fix).

# All v10.0 logic retained where unchanged.
# """
# from __future__ import annotations
...
# # ── Chroma singleton ──────────────────────────────────────────────────────────
# # graph.py — replace the _chroma_client / _tool_collection init block
...
# 2. "SBI Mutual Fund", "HDFC AMC", "Mirae Asset", "Nippon AMC" → always mf_amc.
# 3. "Parag Parikh Flexi Cap", "Axis Bluechip" → always mf_scheme.
# 4. Only assign entity_type=etf when the word ETF appears explicitly in the query.
# 5. Extract ALL named entities from the query.
# 6. First named entity → "primary". Rest → "additional_companies".
# 7. If only one entity → "additional_companies" is [].
# 8. Do NOT include tool_hint here — tools are resolved later per intent.
# """


# def node_classify_and_extract(state: AgentState) -> AgentState:
#     console.print("[Node A] Classify + extract (v10.1 — entity extraction only)")
...
# # ══════════════════════════════════════════════════════════════════════════════
# # Node IQ: Decompose intents  (v10.1 — hardened entity_type rules)
# # ══════════════════════════════════════════════════════════════════════════════
...
#   "intents": [
#     {{
#       "entity":             "<canonical entity name — use the name AS WRITTEN in the query>",
#       "entity_type":        "<stock|mf_scheme|mf_amc|etf|index|general>",
...
# 1. "SBI Mutual Fund", "HDFC AMC", "Mirae Asset", "DSP", "Nippon AMC",
#    "Kotak MF", "Axis AMC", "ICICI Prudential AMC", "Franklin Templeton"
#    → entity_type = mf_amc.  NEVER etf.  NEVER mf_scheme.
# 2. "Parag Parikh Flexi Cap", "Axis Bluechip", "SBI Small Cap Fund",
#    any name ending in "Fund" or "Scheme"
#    → entity_type = mf_scheme.
# 3. entity_type = etf ONLY when the word "ETF" appears explicitly in the
#    user query AND refers to a specific ETF product.
#    "SBI Mutual Fund" does NOT become an ETF even if SBI has ETF products.
# 4. "Reliance", "TCS", "HDFC Bank", "Infosys", "Wipro"
#    → entity_type = stock.
# 5. "Nifty 50", "Sensex", "Bank Nifty"
#    → entity_type = index.

# INTENT DESCRIPTION RULES:
# - Must describe the actual data requested, not a hallucinated entity name.
# - For mf_amc  : "schemes offered by <AMC>" or "fund categories under <AMC>".
# - For mf_scheme: "NAV of <scheme>" or "returns of <scheme>".
# - For stock   : "quarterly results for <company>" or "key ratios for <company>".
# - NEVER invent a specific fund/scheme/ETF name that does not appear in the query.
# - Do NOT rename or expand entity names beyond what the user wrote.

# OTHER RULES:
# 1. Each intent maps to exactly ONE entity and ONE data need.
# 2. If the query has only one entity → return a list of length 1.
# 3. Never merge two different entity families into one intent item.
# 4. For comparisons (X vs Y) create one intent per entity.
# """


# def node_decompose_intents(state: AgentState) -> AgentState:
#     console.print("[Node IQ] Decompose intents (v10.1)")
...
# # ══════════════════════════════════════════════════════════════════════════════
# # Node TVS: Per-intent tool vector search  (v10.1 — returns NEW IntentItem)
# #
# # FIX: _search_for_intent now returns a brand-new IntentItem dict instead
# # of mutating the input object.  This prevents the LangGraph state-snapshot
# # race where matched_tools appeared empty in the router even after TVS
# # successfully found tools.
# # ══════════════════════════════════════════════════════════════════════════════
...
#         """Run Chroma search and return a FRESH IntentItem — never mutate input."""
...
#                 # Error path: also return a fresh dict with empty matched_tools
...
#     # Backfill legacy state fields from the first intent
#     if state["intents"]:
...
# # ══════════════════════════════════════════════════════════════════════════════
# # Node MTCI: MCP Tool Call — per Intent
# # ══════════════════════════════════════════════════════════════════════════════
...
# # ══════════════════════════════════════════════════════════════════════════════
# # Router  (v10.1 — TVS results are authoritative)
# #
# # FIX: mcp_needed is upgraded to True whenever ANY intent has matched_tools.
# # This means the LLM's mcp_needed=False is overridden if TVS already found
# # relevant tools — the query never falls through to the general handler.
# # ══════════════════════════════════════════════════════════════════════════════
...
#     # ── DEBUG (remove once routing is confirmed correct) ──────────────────
...
#     # TVS results are authoritative — upgrade mcp_needed if ANY intent has tools
